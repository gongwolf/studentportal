...
public class AdminDAOImpl implements Schemacode{
	...
	public int setApplicationWindow(int windowID, String program, ProgramBean programBean) {
		...
		sql.append("UPDATE ").append(TABLE_PROGRAM)
				.append(" SET start=?,end=? WHERE window_id = ? AND program=?");
	...
	public List<ProgramBean> getApplicationWindows() {
		...
		return jdbcTemplate.query(sql, new RowMapper<ProgramBean>() {
			@Override
			public ProgramBean mapRow(ResultSet reSet,
	...
	public ProgramBean getApplicationWindow(int windowID, String program) {
		String sql = "SELECT * FROM " + TABLE_PROGRAM
				+ " WHERE window_id = ? AND program = ?";
		 
		try{
			ProgramBean programBean = jdbcTemplate.queryForObject(sql, 
					new Object[]{windowID, program}, 
					new RowMapper<ProgramBean>() {
						@Override
						public ProgramBean mapRow(ResultSet reSet, int rowNum)
								throws SQLException {
							ProgramBean programBean = new ProgramBean(); 
							programBean.setWindowID(reSet.getInt("window_id"));
							String name = reSet.getString("program");
	...
	public ProgramBean getApplicationWindow(String program, String semester, String year) {
		String sql = "SELECT * FROM " + TABLE_PROGRAM
				+ " WHERE program = ? AND semester = ? AND year = ?";
		 
		try{
			ProgramBean programBean = jdbcTemplate.queryForObject(sql, 
					new Object[]{program, semester, year}, 
					new RowMapper<ProgramBean>() {
						@Override
						public ProgramBean mapRow(ResultSet reSet, int rowNum)
								throws SQLException {
	...
	public int addApplicationWindow(ProgramBean programBean){
		...
		try{
			windowID = jdbcTemplate.queryForObject(
					"SELECT window_id FROM " + TABLE_PROGRAM
					+ " WHERE program = ? AND year = ? AND semester = ?", 
					new Object[]{
						programBean.getProgramNameAbbr(), programBean.getYear(), 
						programBean.getSemester()
					}, Integer.class); 
		}catch(EmptyResultDataAccessException e){
			// insert
			String startDate = Parse.FORMAT_DATETIME.format(programBean.getStartDate());
			...
			sql = "INSERT INTO " + TABLE_PROGRAM + " (program, year, semester, start, end)"
	                + " VALUES (?, ?, ?, ?, ?)";
			params = new Object[]{programBean.getProgramNameAbbr(), programBean.getYear(), 
					programBean.getSemester(), startDate, endDate};
	...
	public Map<String, List<ProgramBean>> getProgramDeadlines() {
		...
		sql.append("SELECT * FROM ")
			.append(TABLE_PROGRAM).append(" AS p1 ")
		...
		return jdbcTemplate.query(sql.toString(), new ResultSetExtractor<Map<String, List<ProgramBean>>>() {
			@Override
			public Map<String, List<ProgramBean>> extractData(ResultSet rs)
	...
	public List<ApplicationBean> getApplicantsByPerson(
		...
		if (!firstName.isEmpty() || !lastName.isEmpty() || !birthDate.isEmpty()
			...
			if (!firstName.isEmpty()) {
				sql.append(" first_name LIKE '%" + firstName
						+ "%' AND");
			}

			if (!lastName.isEmpty()) {
				sql.append(" last_name LIKE '%" + lastName
						+ "%' AND");
			}
			...
			if (!birthDate.isEmpty()) {
				sql.append(" DATE_FORMAT(birth_date, '%m/%d/%Y')='"
								+ birthDate + "' AND");
			}
	...
	public List<ProfileBean> getStudentProfile(String[] userIDList) {
		...
		return jdbcTemplate.query(sql.toString(),
				new ResultSetExtractor<List<ProfileBean>>(){

				@Override
				public List<ProfileBean> extractData(ResultSet reSet)
						throws SQLException, DataAccessException {
	...
	public List<SelfReportBean> getSelfreportWindows(boolean isActive) {
		...
		return jdbcTemplate.query(sql, new RowMapper<SelfReportBean>() {
			@Override
			public SelfReportBean mapRow(ResultSet reSet,
					int rowNum) throws SQLException {
	...
	public int addSelfreportWindow(SelfReportBean selfReportBean) {
		try{
			jdbcTemplate.queryForObject(
					"SELECT window_id FROM " + TABLE_SELFREPORT_MANAGE
					+ " WHERE semester = ?", new Object[]{selfReportBean.getSemester()}, Integer.class); 
		}catch(EmptyResultDataAccessException e){
			// insert
			String sql = "INSERT INTO " + TABLE_SELFREPORT_MANAGE + " (semester, start, end)"
				                + " VALUES (?, ?, ?)";
			return jdbcTemplate.update(sql, new Object[]{selfReportBean.getSemester(), 
					selfReportBean.getStartDate(), selfReportBean.getEndDate()}); 
		}
		return 0; 
	}
	...
	public int setSelfreportWindow(int windowID, SelfReportBean selfReportBean) {
		StringBuilder sql = new StringBuilder();
		sql.append("UPDATE ").append(TABLE_SELFREPORT_MANAGE)
				.append(" SET start=?,end=? ")
				.append("WHERE window_id=? AND semester=?");

		return jdbcTemplate.update(sql.toString(), new Object[]{
			selfReportBean.getStartDate(), selfReportBean.getEndDate(), 
			windowID, selfReportBean.getSemester()
		}); 
	}
	
	public boolean isSelfreportWindowExist(int windowID, String semester){
		try{
			String sql = "SELECT window_id FROM " + TABLE_SELFREPORT_MANAGE
					+ " WHERE window_id = ? AND semester = ?";
			return jdbcTemplate.query(sql, new Object[]{windowID, semester}, 
	...
	public List<SelfReportBean> getSubmittedSelfReport(String windowID, String[] userIDs) {
		...
		return jdbcTemplate.query(sql.toString(), new Object[]{windowID}, 
				new RowMapper<SelfReportBean>() {
	 ...
	 ****************************************************/
	public List<ApplicationBean> getApplicationsBySchoolFromTo(
			String schoolFrom, String schoolTarget,
			int year, String semester, String program, boolean completedOnly) {
		...
		*/
		...
		case ProgramCode.TRANS:
			...
			sql.append("(SELECT * FROM ").append(TABLE_APPLICATION_LIST).append(" WHERE program = ? AND school_year=? AND school_semester=?) AS a\n");
				...
				while(reSet.next()){
					...
					if (programEverIn != null) { // *
						...
						if (ampScholarship.equals("1")) {
							...
							involvementBean.setAmpScholarshipSemester(reSet.getString("amp_scholarship_semester"));
							involvementBean.setAmpScholarshipYear(reSet.getString("amp_scholarship_year"));
							involvementBean.setAmpScholarshipAmount(Float
									.parseFloat(reSet.getString("amp_scholarship_amount")));
						}
	...
	public List<ApplicationBean> getSelectedApplicationForExcel(int year, String semester, 
			String program, List<String> ids) {
	...
	private List<ApplicationBean> getSelectedApplicationCCCONF(StringBuilder sql,
			int year, String semester, String program){
		return jdbcTemplate.query(sql.toString(), 
			new ResultSetExtractor<List<ApplicationBean>>(){
	...
	private List<ApplicationBean> getSelectedApplicationMESA(StringBuilder sql,
			int year, String semester, String program){
		return jdbcTemplate.query(sql.toString(), 
			new ResultSetExtractor<List<ApplicationBean>>(){
	...
	private List<ApplicationBean> getSelectedApplicationTRANS(StringBuilder sql,
			int year, String semester, String program){
		return jdbcTemplate.query(sql.toString(), 
			new ResultSetExtractor<List<ApplicationBean>>(){
	...
	private List<ApplicationBean> getSelectedApplicationSCCORE(StringBuilder sql,
			int year, String semester, String program){
		return jdbcTemplate.query(sql.toString(), 
			new ResultSetExtractor<List<ApplicationBean>>(){
			...
			public List<ApplicationBean> extractData(ResultSet reSet)
				...
				while(reSet.next()){
					...
					// get academicInfo
					AcademicBean academicBean = null;
					// ** don't use academic_school to check null, as it is filled as first step of application
					Date academicGradDate = reSet.getDate("academic_grad_date");
					if (academicGradDate != null) {
						academicBean = new AcademicBean();
                        String academicSchool = reSet.getString("academic_school");
	...
	private List<ApplicationBean> getSelectedApplicationURS(StringBuilder sql,
			int year, String semester, String program){
		return jdbcTemplate.query(sql.toString(), 
			new ResultSetExtractor<List<ApplicationBean>>(){
	...
	private List<ApplicationBean> getSelectedApplicationIREP(StringBuilder sql,
			int year, String semester, String program){
		return jdbcTemplate.query(sql.toString(), 
			new ResultSetExtractor<List<ApplicationBean>>(){
	 ...
	 ****************************************************/
	public void batchUpdateApplicationDecision(String program, List<String> appIDs, 
			String decision, String schoolTarget){
		String sql = "UPDATE " + TABLE_APPLICATION_LIST
				+ " SET decision=?, accept_school = ?, accept_status = ? "
	 ...
	 ****************************************************/
	public List<ApplicationBean> getSelectedApplicationDecision(int year, String semester, 
			String program, List<String> ids) {
		StringBuilder sql = new StringBuilder();
	...
	public List<ApplicationBean> getDecisionProfile(int year, String semester, 
			String program, String[] appIDList) {
		List<String> ids = new ArrayList<>(Arrays.asList(appIDList)); 
		StringBuilder sql = new StringBuilder();
	...
	public List<ApplicationBean> getAcceptedURSProject(String schoolSemester, int schoolYear, String schoolTarget){
		...
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT a.application_id,school_year,school_semester,mentor_id,mentor_first_name,mentor_last_name,c.first_name,c.last_name,project_title \n");
		sql.append("FROM ").append(TABLE_APPLICATION_LIST).append(" a \n");
	...
	public List<ApplicationBean> getApplicationNMSUforCommitteeReview(String program, 
			List<String> ids) {
	...
	public int addEvaluation(String applicationID, EvaluationBean evaluationBean) {
		...
		String table = evaluationBean.getEvalPoint()
				.equalsIgnoreCase(ProgramCode.EVALUATION_POINT.MIDTERM.toString())
				? TABLE_EVAL_MIDTERM : TABLE_EVAL_END; 
		...
		try{
			jdbcTemplate.queryForObject(
					...
					new Object[]{
						applicationID, evaluationBean.getEvalYear(), 
						evaluationBean.getEvalTerm()
					}, Integer.class); 
		}catch(EmptyResultDataAccessException e){
			// insert
			String endDate = Parse.FORMAT_DATETIME.format(evaluationBean.getEvalDeadline());
			endDate = endDate.split("\\s+")[0] + " 23:59:59"; 
			String sql = "INSERT INTO " + table + " (application_id, eval_year, eval_semester, mentee_id, mentor_id, deadline)"
	                + " VALUES (?, ?, ?, ?, ?, ?)";
			Object[] params = new Object[]{applicationID, evaluationBean.getEvalYear(), 
					evaluationBean.getEvalTerm(),
	...
	public List<EvaluationBean> getEvaluations(int applicationID) {
		...
		sql.append("SELECT eval_year,eval_semester,deadline,notified_date,submit_date, 'End-Of-Semester' as eval_point \n");
	...
	public EvaluationBean getEvalutionBasic(String applicationID, 
			String evalYear, String evalSemester, String evalPoint){
		...
		try{
			EvaluationBean evaluationBean = jdbcTemplate.queryForObject(sql.toString(), 
				new Object[]{applicationID, evalYear, evalSemester}, 
				new RowMapper<EvaluationBean>() {
					@Override
					public EvaluationBean mapRow(ResultSet reSet, int rowNum)
							throws SQLException {
						...
						bean.setMentorName(reSet.getString("mentor_first_name") + " " + reSet.getString("mentor_last_name"));
	...
	public int setEvaluationNotifiedDate(String applicationID, 
			String evalYear, String evalSemester, String evalPoint) {
		String table = ProgramCode.TABLE_EVALUATION.get(evalPoint);
		StringBuilder sql = new StringBuilder();
	...
	public int setEvaluationDeadline(String applicationID, 
			String evalYear, String evalSemester, String evalPoint, Date deadline) {
		String table = ProgramCode.TABLE_EVALUATION.get(evalPoint);
		StringBuilder sql = new StringBuilder();
		sql.append("UPDATE ").append(table).append(" SET deadline=? ")
				.append("where application_id = ? AND eval_year = ? AND eval_semester = ? \n"); 
		return jdbcTemplate.update(sql.toString(), new Object[]{
			deadline, applicationID, evalYear, evalSemester
		}); 
	}
	...
	public List<List<String>> getEvaluationsMidTerm(List<String> ids) {
		String table = ProgramCode.TABLE_APPLICATION_DETAIL.get(ProgramCode.URS);
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT a.*, b.project_title,c.first_name,c.last_name,d.mentor_first_name,d.mentor_last_name,d.mentor_email,d.mentor_department \n");
		...
		return jdbcTemplate.query(sql.toString(), new Object[]{}, 
				new RowMapper<List<String>>() {
					@Override
					public List<String> mapRow(ResultSet reSet, int rowNum)
							throws SQLException {
	...
	public List<String> getEvaluationMidTerm(String applicationID, 
			String evalYear, String evalSemester, String evalPoint){
		String table = ProgramCode.TABLE_APPLICATION_DETAIL.get(ProgramCode.URS);
	...
	public List<String> getEvalutionEnd(String applicationID, 
			String evalYear, String evalSemester, String evalPoint){
		String table = ProgramCode.TABLE_APPLICATION_DETAIL.get(ProgramCode.URS);
	...
	public List<EvaluationBean> getEvaluationsStu(int applicationID) {
		...
		return jdbcTemplate.query(sql.toString(), new Object[]{applicationID},
				new RowMapper<EvaluationBean>() {
					@Override
					public EvaluationBean mapRow(ResultSet reSet, int rowNum)
							throws SQLException {
	...
	public int addStudentEvaluation(String applicationID, EvaluationBean evaluationBeanStu) {
		...
		String table = evaluationBeanStu.getEvalPoint()
				.equalsIgnoreCase(ProgramCode.EVALUATION_POINT.MIDTERM.toString())
				? TABLE_EVAL_MIDTERM_STU : TABLE_EVAL_END_STU; 
		...
		try{
			jdbcTemplate.queryForObject(
					...
					+ " WHERE application_id = ? AND eval_year = ? AND eval_semester = ?", 
					new Object[]{
						applicationID, evaluationBeanStu.getEvalYear(), 
						evaluationBeanStu.getEvalTerm()
					}, Integer.class); 
		}catch(EmptyResultDataAccessException e){
			// insert
			...
			Object[] params = new Object[]{applicationID, evaluationBeanStu.getEvalYear(), 
					evaluationBeanStu.getEvalTerm(),
					evaluationBeanStu.getMenteeID(), evaluationBeanStu.getMentorID(), endDate}; 
			return jdbcTemplate.update(sql, params); 
		}
	...
	public int setEvaluationDeadlineStu(String applicationID, 
			String evalYear, String evalSemester, String evalPoint, Date deadline) {
	...
	public EvaluationBean getEvalutionBasicStu(String applicationID, 
			String evalYear, String evalSemester, String evalPoint){
		String table = ProgramCode.EVALUATION_POINT.MIDTERM.toString().equalsIgnoreCase(evalPoint)
				? TABLE_EVAL_MIDTERM_STU : TABLE_EVAL_END_STU;
		StringBuilder sql = new StringBuilder();
	...
	public int setEvaluationNotifiedDateStu(String applicationID, 
			String evalYear, String evalSemester, String evalPoint) {
		String table = ProgramCode.EVALUATION_POINT.MIDTERM.toString().equalsIgnoreCase(evalPoint)
				? TABLE_EVAL_MIDTERM_STU : TABLE_EVAL_END_STU;
		StringBuilder sql = new StringBuilder();
	...
	public List<String> getEvaluationMidTermStu(String applicationID, 
			String evalYear, String evalSemester, String evalPoint){
		String table = ProgramCode.TABLE_APPLICATION_DETAIL.get(ProgramCode.URS);
